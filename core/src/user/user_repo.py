    ...
    async def user_get(self) -> List[UserGetSchema]:
        result = (
            await self.session.execute(
                ...
                .outerjoin(DepartmentOrm, UserOrm.department_id == DepartmentOrm.id)
                .outerjoin(SchoolOrm, UserOrm.school_id == SchoolOrm.id)
                .outerjoin(
                    FieldOfStudyOrm, UserOrm.field_of_study_id == FieldOfStudyOrm.id
                )
            )
        ).all()
